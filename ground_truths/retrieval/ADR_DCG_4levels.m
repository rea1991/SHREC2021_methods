function [CG,DCG,NCG,NDCG,ADR] = ADR_DCG_4levels(A,rilevanza)



for i=1:size(rilevanza,1)
    v_irel(i)=length(find(rilevanza(:,i)==3));
...
for i=1:size(rilevanza,1)
    vec=[3*ones(1,v_irel(i)) 2*ones(1,v_hrel(i)) ones(1,v_rel(i)) zeros(1,size(rilevanza,1)-v_rel(i)-v_hrel(i)-v_irel(i))];
    ideal_gain(i,:)=vec;
end
...
for i=v_irel(j)+1:v_hrel(j)+v_irel(j)
  if gain(j,i)==2
     foundh=foundh+1;
  end
  if gain(j,i) >=1
...
for i=(v_hrel(j)+v_irel(j)+1):(v_hrel(j)+v_irel(j)+v_rel(j))
  if gain(j,i) >=1
     foundr=foundr+1;
  end
  ADRv(j,i)=foundr/i;
...
ADR=[];
for j=1:size(rilevanza,1)
ADR(j)=(1/(v_irel(j)+v_hrel(j)+v_rel(j)))*sum(ADRv(j,:));
end
ADR=ADR';
end
